...
classify_biomes <- function(data = NULL, readOutput = F, folder = NULL, files = NULL, 
  ...
  fpc_tree_total <- apply(
    lpjmlKit::subset_array(fpc, list(band = fpc_trees)),
    c("cell"),
    sum,
    na.rm = TRUE
  )
  ...
  fpc_tree_tropical <- apply(
    lpjmlKit::subset_array(fpc, list(band = fpc_tropical_trees)),
    c("cell"),
    sum,
    na.rm = TRUE
  )
  fpc_tree_temperate <- apply(
    lpjmlKit::subset_array(fpc, list(band = fpc_temperate_trees)),
    c("cell"),
    sum,
    na.rm = TRUE
  )
  fpc_tree_boreal <- apply(
    lpjmlKit::subset_array(fpc, list(band = fpc_boreal_trees)),
    c("cell"),
    sum,
    na.rm = TRUE
  )
  fpc_tree_needle <- apply(
    lpjmlKit::subset_array(fpc, list(band = fpc_needle_trees)),
    c("cell"),
    sum,
    na.rm = TRUE
  )
  fpc_tree_evergreen <- apply(
    lpjmlKit::subset_array(fpc, list(band = fpc_evergreen_trees)),
    c("cell"),
    sum,
    na.rm = TRUE
  )
  fpc_grass_total <- apply(
    lpjmlKit::subset_array(fpc, list(band = fpc_grass)),
    c("cell"),
    sum,
    na.rm = TRUE
  )
  fpc_total <- apply(
    lpjmlKit::subset_array(fpc,
  ...
  is_temperate_woody_savannah <- {
    is_woody_savannah &
      lpjmlKit::subset_array(fpc, list(band = "Temperate C3 Grass")) >
      lpjmlKit::subset_array(fpc, list(band = "Tropical C4 Grass")) &
      temp >= 0 #-2 &
    #latitudes < 55
  }
  # Warm Woody Savanna, Woodland & Shrubland
  ...
  # Water body
  is_water <- {
    lpjmlKit::subset_array(fpc, list(band = "natvegfrac")) == 0
  }
