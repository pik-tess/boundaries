...
calc_greenwater_status <- function(path_scenario,
  ...
  rootmoist_reference %<-% tmp_read_monthly(
  ...
  rootmoist_reference <- brick(paste0(path_reference,
                                     "/",
                                     prefix_monthly_output,
                                     "mrootmoist.nc"))
  # -------------------------------------------------------------------------- #
  ...
  # TO BE REPLACED BY lpjmlKit::read_output ---------------------------------- #
  #   hardcoded values to be internally replaced
  rootmoist_scenario %<-% tmp_read_monthly(
  ...
  rootmoist_scenario <- brick(paste0(path_scenario,
                                      "/",
                                      prefix_monthly_output,
                                      "mrootmoist.nc"))
  # -------------------------------------------------------------------------- #
  ...
  calc_GW_baseline(rootmoist_reference)
  
  # calculate GW dry & wet departures and return df with percentage 
  # of annual area of departure
  
  # read area file:
  zz            <- file(paste0("p/projects/open/sibyll/tools/area.bin"),"rb")
  area          <- readBin(zz, numeric(), n= 67420, size=4)
  close(zz)
  remove(zz)
  # create raster
  area          <- area* 10^-6  #m^2 to km^2
  arearaster    <- lpjmlRaster(area,gridf) #rasterlayer of cell area
  remove(area)
  # calculate the total area
  gw_area <- cellStats(arearaster, stat = 'sum')
  
  calc_GW_depart(rootmoist_scenario,arearaster,gw_area,q5_base,q95_base)
...
calc_GW_baseline <- function(rootmoist_reference) {

  #nyr of reference baseline
  nyr_base <- nlayers(rootmoist_reference)/12
  ...
  for (j in 1:nyr_base) {
    ...
    file_j   <- rootmoist_reference[[index_j]]
...
calc_GW_depart <- function(rootmoist_scenario,arearaster,gw_area,q5_base,q95_base) {
  
  #nyr variables
  nyr_scn <- nlayers(rootmoist_scenario)/12
  firstyr <- labels(rootmoist_scenario[[1]])
  firstyr <- as.numeric(substr(firstyr,2,5))
  
  #create departure data.frame
  departure.df   <- matrix(nrow = nyr_scn,ncol = 3) 
  departure.df[,1] <- firstyr:(nyr_scn+firstyr-1) #create year col
  ...
  for (i in 1:nyr_scn) {
    ...
    file_i   <- rootmoist_scenario[[index_i]]
